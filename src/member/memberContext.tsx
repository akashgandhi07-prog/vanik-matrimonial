    ...
    const run = async () => {
      ...
      try {
        ...
        > => {
          try {
            ...
            if (boot.profile) {
              ...
              privateFromBootstrap = (boot.member_private ??
        ...
        if (
          ...
          p.status === "rejected" ||
          profileNeedsMembershipExpiredRoute(p)
        ) {
          return;
        }
        ...
        setFeedbackKeys(
          new Set(
            (fbRows ?? []).map(
              (r) => `${r.request_id as string}:${r.candidate_id as string}`,
            ),
          ),
        );
      } catch (e) {
  ...
  if (!profile) {
    return (
      <div
        ...
        style={{ padding: "48px 16px", maxWidth: 560 }}
      >
