...
Deno.serve(async (req) => {
  ...
  const { data: runRow } = await admin
    .from('cron_job_runs')
    .insert({ job_name: 'send-feedback-reminders', status: 'running', triggered_by: 'schedule' })
    .select('id')
    .single();
  ...
  try {
    ...
    if (runId) {
      await admin
        .from('cron_job_runs')
        .update({ status: 'success', finished_at: new Date().toISOString(), result: { emails_sent: emails } })
        .eq('id', runId);
    }
